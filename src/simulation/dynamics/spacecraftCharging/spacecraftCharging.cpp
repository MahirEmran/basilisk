...
void SpacecraftCharging::writeOutputStateMessages(uint64_t clockTime) {
    ...
    servicerPECurrentMsgBuffer = this->servicerPECurrentOutMsg.zeroMsgPayload;
    servicerPECurrentMsgBuffer.current = this->servicerPECurrent;
    this->servicerPECurrentOutMsg.write(&servicerPECurrentMsgBuffer, this->moduleID, clockTime);
    ...
    targetPECurrentMsgBuffer = this->targetPECurrentOutMsg.zeroMsgPayload;
    targetPECurrentMsgBuffer.current = this->targetPECurrent;
    this->targetPECurrentOutMsg.write(&targetPECurrentMsgBuffer, this->moduleID, clockTime);
}
...
void SpacecraftCharging::equationsOfMotion(double integTimeSeconds, double timeStep) {
    ...
    servicerPotentialRate(0, 0) = (this->servicerEBCurrent + this->servicerPECurrent) / this->servicerCapacitance;
    ...
    targetPotentialRate(0, 0) = (this->targetEBCurrent + this->targetPECurrent) / this->targetCapacitance;
...
void SpacecraftCharging::computePhotoelectricCurrent() {
    ...
    if (this->servicerPotential <= 0) {
        this->servicerPECurrent = fluxPhotons * this->servicerSunlitArea;
    } else {
        this->servicerPECurrent = fluxPhotons * this->servicerSunlitArea * exp(-1 * this->servicerPotential / tempPhotons);
    }

    // Compute the target photoelectric current
    if (this->targetPotential <= 0) {
        this->targetPECurrent = fluxPhotons * this->targetSunlitArea;
    } else {
        this->targetPECurrent = fluxPhotons * this->targetSunlitArea * exp(-1 * this->targetPotential / tempPhotons);
    }
}
