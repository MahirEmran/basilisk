...
class ActiveGuidance(sysModel.SysModel):
    ...
    def UpdateState(self, CurrentSimNanos):
        ...
        if self.mode == "ROLL_ONLY":
            ...
            # Math change: EXPERIMENT roll now optimizes LOST clearance against
            # Earth + Sun + Moon (when available), not just Earth + Sun.
            # This aligns guidance objective with uptime metrics, which already
            # treated Moon as a LOST keep-out body.
            ...
            if can_charge:
                ...
                x_B = compute_compromise_x(earth_hat_found, sun_hat_found, self.rho_rad)
                # Keep fallback behavior consistent with explicit EXPERIMENT mode.
                extra_keepout_hats = [moon_hat_sc] if moon_hat_sc is not None else None
                best_roll_deg, best_y, best_z, _ = solve_roll_for_lost_clearance(
                    x_B,
                    earth_hat_sc,
