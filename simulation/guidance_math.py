...
def solve_roll_for_lost_clearance(x_b, earth_hat, sun_hat, prev_roll_deg=None, extra_keepout_hats=None):
    ...
    # Score = minimum separation across all keep-out bodies, so maximizing score protects worst case.
    for roll_deg in range(0, 360, 2):
        ...
        # Core math change:
        # Old objective used only [Earth, Sun].
        # New objective uses [Earth, Sun, ...extra bodies (Moon)] and maximizes
        # the worst-case angle. This min-max formulation directly lifts whichever
        # body is currently the tightest keep-out constraint.
